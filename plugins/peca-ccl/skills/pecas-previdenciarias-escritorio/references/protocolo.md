...
- **Exceção: procuração é sempre o item 3, posição fixa.** Diferente das
  ...
  "Categoria ausente na pasta" (ver INSTRUÇÃO DE INÍCIO no SKILL.md) e
  pergunte ao usuário se ele vai enviar o documento agora ou se confirma que
  já existe procuração válida nos autos — nesse segundo caso, a posição 3
  fica reservada na numeração sem exigir o arquivo físico na pasta.
...
- **Entrega em arquivos individuais, nunca em zip.** A entrega dos
  ...
  aberto e protocolado isoladamente, sem exigir extração prévia.
